...
export async function search_imdb(query) {
  ...
  let imdb_search = await fetch(`https://v2.sg.media-imdb.com/suggestion/${encodeURIComponent(query.slice(0, 1))}/${encodeURIComponent(query)}.json`)
  if (!imdb_search.ok) {
    return {
      error: `IMDb search request failed: ${imdb_search.status} ${imdb_search.statusText}`
    }
  }
...
export async function gen_imdb(sid) {
  ...
  if (!imdb_page_resp.ok) {
    return Object.assign(data, {
      error: `IMDb request failed: ${imdb_page_resp.status} ${imdb_page_resp.statusText}`
    });
  }
  ...
  let json_ld_script = $('script[type="application/ld+json"]').html();
  if (!json_ld_script) {
    return Object.assign(data, {
      error: "IMDb page parse failed: JSON-LD script not found"
    });
  }
  let page_json;
  try {
    page_json = JSON.parse(json_ld_script.replace(/\n/ig, ''));
  } catch (e) {
    return Object.assign(data, {
      error: "IMDb page parse failed: JSON-LD parse error"
    });
  }
  ...
  let total_data = {};
  let next_data_raw = $('script#__NEXT_DATA__').html();
  if (next_data_raw) {
    try {
      let next_data_json = JSON.parse(next_data_raw.replace(/\n/ig, ''));
  ...
  if (imdb_release_info_page_resp.ok) {
