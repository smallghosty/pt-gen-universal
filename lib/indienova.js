...
export async function gen_indienova(sid, config = {}) {
  ...
  let small_elem = title_field.find("h1 small");
  let span_elem = title_field.find("h1 span");
  data["another_title"] = small_elem.length ? small_elem.text().trim() : "";
  data["english_title"] = span_elem.length ? span_elem.text().trim() : "";
  ...
  data["level"] = level_field.length ? level_field.find("img").map(function () {
    return $(this).attr("src");
  }).get() : [];

  // 提取价格信息
  let price_fields = $("ul.db-stores");
  data["price"] = price_fields.length ? price_fields.find("li").map(function () {
  ...
  if (data["links"] && Object.keys(data["links"]).length > 0) {
    const format_links = Object.entries(data["links"])
      .map(([key, value]) => `[url=${value}]${key}[/url]`);
